...
#' @param C_prop The proportion of active off-diagonal dimensions where the covariance changes occur (Default: 0.1).
...
#'   \item{A}{A numeric matrix with 2 columns indicating the (row, col) indices of the active off-diagonal coordinates.}
#' }
#'
...
#' set.seed(2026)
#' # 1. Base Process
#' dat_base <- generate_cov_data(n = 300, p = 100, T_cp = 5)
#'
#' # 2. Spatial Correlation
#' dat_spatial <- generate_cov_data(n = 300, p = 100, T_cp = 5, rho = 0.6)
#'
#' # 3. Vanishing Signals
#' dat_vanish <- generate_cov_data(n = 300, p = 100, T_cp = 5, vanishing = TRUE)
generate_cov_data <- function(n = 500, p = 500, T_cp = 10, C_prop = 0.1,
  ...
  for (t in 1:T_cp) {
    ...
    if (s0 > 0) {
      ...
      jump_vals <- (signal * sign_mult * sd_vec[A[, 1]] * sd_vec[A[, 2]]) / sqrt(min_dist)
