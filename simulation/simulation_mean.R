...
# 1개의 세팅을 10개의 청크로 나눔 (1 청크당 10 reps)
...
p_vec <- c(200, 500)
T_vec <- c(5, 10)
CA_vec <- c(0.1, 0.25, 0.5)
Omega_vec <- c("sparse", "dense")
CS_vec <- c(0.25, 0.5, 0.75, 1, 1.25, 1.5)

# 총 144개의 세팅 (1440 Tasks)
param_grid <- expand.grid(p = p_vec, T = T_vec, CA = CA_vec, Omega = Omega_vec, CS = CS_vec)
current_setting <- param_grid[setting_id, ]

p_val <- current_setting$p
T_val <- current_setting$T
CA_val <- current_setting$CA
Omega_val <- as.character(current_setting$Omega)
CS_val <- current_setting$CS

# 폴더 사전 생성
save_dir <- "results_mean"
if (!dir.exists(save_dir)) dir.create(save_dir, recursive = TRUE, showWarnings = FALSE)

for (rep_id in rep_start:rep_end) {

  save_path <- sprintf("%s/sim_set_%02d_rep_%03d.rds", save_dir, setting_id, rep_id)
  ...
  tryCatch({
    # Simulation
    dat <- generate_mean_data_fixed(
      n = n, p = p_val, T_cp = T_val, C_A = CA_val,
      delta_n = delta_n, C_S = CS_val, omega_type = Omega_val
    )

    # Proposed Method
    res_hdbcp <- mxPBF_mean(X = dat$X,
                            beta = 0.875,
                            m_min = 25,
                            FPR_0 = 0.05,
                            C_conv = 0.1,
    ...
    ## Inspect method (p 대신 p_val 사용)
    out <- capture.output(res_inspect <- inspect(t(dat$X), threshold = compute.threshold(n, p_val, show_progress = F), show_progress = F))
    raw_inspect <- prune_inspect(res_inspect$changepoints, delta_n)
    eta_hat_inspect <- as.numeric(unlist(raw_inspect))
    metric_inspect <- evaluate_cpd(eta_hat = eta_hat_inspect, eta_true = dat$eta, n = n)
